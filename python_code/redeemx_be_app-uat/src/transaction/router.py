...
def user_vendor_transaction(request:Request, transaction:TransactionUserInputSchema, response:Response, session=session, auth_user=auth_user):
    ...
    if auth_user.get("is_user"):
        ...
        if not user_points:
            ...
            return RestResponse(error="Insufficient points")
...
async def upload_file(
    ...
    if auth_user.get("is_admin"):
        ...
        # Allow CSV, XLSX, and XLS file formats.
        if not file.filename.endswith((".csv", ".xlsx", ".xls")):
            response.status_code = 400
            logger.error(f"Invalid file format attempted: {file.filename}")
            return RestResponse(error="Invalid file format. Please upload a CSV, XLSX, or XLS file.")

        if file.filename.endswith(".csv"):
            df = pd.read_csv(file.file, header=8)
        else:
            df = pd.read_excel(file.file, header=8)


        required_columns = ["E. Code", "Name", "Status"]
        for col in required_columns:
            if col not in df.columns:
                response.status_code = 400
                logger.error(f"Missing '{col}' column in uploaded file.")
                return RestResponse(error=f"Missing required column '{col}' in the file.")

        # Remove rows where the status is "Absent" (or is NaN)
        df = df[df["Status"].notna() & (df["Status"].astype(str).str.strip() != "Absent")]
        points_to_assign = 14

        # Remove duplicate entries based on "E. Code" (assuming Emp Name remains same)
        df_unique = df.drop_duplicates(subset=["E. Code"])
        
        # Create a mapping from employee code to employee name from the file.
        emp_info = df_unique.set_index("E. Code")["Name"].to_dict()

        # Extract unique employee IDs from the file.
        excel_emp_ids = list(emp_info.keys())
        user_list = session.exec(select(User).filter(User.is_user == True)).all()
        user_dict = {user.emp_id: user for user in user_list}
        data = {"assigned": [], "unassigned": []}
        transactions_added = 0

        for emp in excel_emp_ids:
            emp_name = emp_info.get(emp, "Unknown")
            if emp in user_dict:
                user_obj = user_dict[emp]
                transaction = Transaction(user_id=user_obj.id, points=points_to_assign)
                session.add(transaction)
                transactions_added += 1
                data["assigned"].append({
                    "emp_id": emp,
                    "emp_name": emp_name,
                    "details": f"{points_to_assign} points assigned"
                })
            else:
        ...
        if data.get("unassigned"):
            logger.warning(f"Unassigned users from file '{file.filename}': {data['unassigned']}")

        return RestResponse(data=data, message="File uploaded successfully")
