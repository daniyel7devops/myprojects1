...
def delete_vendor(
    ...
    session.commit()
    return RestResponse(message="Vendor deleted successfully")
...
def get_vendor_details(response:Response, request:Request, session=session, auth_user=auth_user):
    ...
    vendor_details = session.exec(select(Vendor, User).join(User).where(or_(Vendor.user_id == auth_user.get("user_id"),auth_user.get("is_admin")))).first()
